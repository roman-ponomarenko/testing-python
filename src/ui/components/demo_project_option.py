...
class DemoProjectOption:
    ...
    def get_name_text(self) -> str:
        return self._root.locator(".items-center").inner_text().strip()

    def select(self) -> None:
        self._root.click()

    def verify_is_selected(self) -> None:
        expect(self.selector).to_have_attribute("src", re.compile(r"circle-tick.svg"))

    def verify_is_not_selected(self) -> None:
        expect(self.selector).to_have_attribute("src", re.compile(r"circle.svg"))
